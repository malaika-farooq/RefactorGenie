...
def get_functionality():
    options = {
        "Refactor Code": "arrow-clockwise",
        "Upload File and Refactor": "plus-circle"
    }
...
def process_code(selected_functionality: str, refactor_options: Dict) -> ArcticOps:
    ...
    with col1:
        with st.container(border=True):
            if selected_functionality == "Refactor Code":
                st.subheader("Enter your text here and Refactor Code")
            else:
                st.subheader(
            ...
            user_input = (
                ...
                if selected_functionality in ["Refactor Code", "Review Code"]
                else st.text_area("Enter your requirements here", height=200)
            )

            button_label = "Refactor Code", [selected_functionality]
